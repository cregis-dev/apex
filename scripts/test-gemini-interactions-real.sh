...
if [[ -z "$GEMINI_API_KEY" ]]; then
  echo "[gemini-interactions-real] missing GEMINI_API_KEY."
  echo "[gemini-interactions-real] Put it in .env.gemini-native or export it before running:"
  echo "  GEMINI_API_KEY=... ./scripts/test-gemini-interactions-real.sh"
  exit 1
fi

RUNTIME_DIR="$ROOT_DIR/.run/e2e/gemini-native"
CONFIG_PATH="$RUNTIME_DIR/generated.gemini-interactions.config.json"
ENV_OUT="$RUNTIME_DIR/generated.gemini-interactions.env"
SERVER_LOG="$RUNTIME_DIR/interactions-server.log"
OUTPUT_PATH="${APEX_GEMINI_INTERACTIONS_OUTPUT:-$RUNTIME_DIR/interactions-response.json}"
mkdir -p "$RUNTIME_DIR"
...
echo "[gemini-interactions-real] generating Apex config"
...
echo "[gemini-interactions-real] starting Apex on $LISTEN"
...
echo "[gemini-interactions-real] running interactions client"
APEX_BASE_URL="http://$LISTEN" \
APEX_TEAM_KEY="$TEAM_KEY" \
APEX_GEMINI_INTERACTIONS_OUTPUT="$OUTPUT_PATH" \
PYTHONDONTWRITEBYTECODE=1 \
python3 tests/e2e/gemini_interactions_client.py "$@"

echo "[gemini-interactions-real] OK"
