...
| previous_retrospective | Previous epic's retrospective (optional) | `{implementation_artifacts}/retrospectives/epic-{{prev_epic_num}}-retro-*.md` | SELECTIVE_LOAD |
...
<action>Scan {implementation_artifacts}/stories for highest numbered story files</action>
...
<action>For each story in epic {{epic_number}}, read the complete story file from {implementation_artifacts}/stories/{{epic_number}}-{{story_num}}-*.md</action>
...
<check if="{{prev_epic_num}} >= 1">
  <action>Search for previous retrospectives using pattern: {implementation_artifacts}/retrospectives/epic-{{prev_epic_num}}-retro-*.md</action>
