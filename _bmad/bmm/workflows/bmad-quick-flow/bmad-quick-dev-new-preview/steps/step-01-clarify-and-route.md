---
wipFile: '{implementation_artifacts}/tech-specs/tech-spec-wip.md'
deferred_work_file: '{implementation_artifacts}/tech-specs/deferred-work.md'
...
- Active specs (`ready-for-dev`, `in-progress`, `in-review`) in `{implementation_artifacts}/tech-specs`? → List them and HALT. Ask user which to resume (or `[N]` for new).
...
- Unformatted spec or intent file lacking `status` frontmatter in `{implementation_artifacts}/tech-specs`? → Suggest to the user to treat its contents as the starting intent for this workflow. DO NOT attempt to infer a state and resume it.

## INSTRUCTIONS

1. Load context.
   - List files in `{planning_artifacts}` and `{implementation_artifacts}/tech-specs`.
...
5. Generate `spec_file` path:
   ...
   - If `{implementation_artifacts}/tech-specs/tech-spec-{slug}.md` already exists, append `-2`, `-3`, etc.
   - Set `spec_file` = `{implementation_artifacts}/tech-specs/tech-spec-{slug}.md`.
6. Route:
