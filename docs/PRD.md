...
- **部署模式**：单机部署 / Sidecar
- **支持**：Web Dashboard 管理界面（展示 Usage 记录与 Metrics 指标）
...
- **日志 (Logging)**：
    - 结构化日志输出，支持文件和标准输出。
- **数据存储 (Data Storage)**：
    - 使用 SQLite 数据库持久化存储 Usage 记录与 Metrics 事件
    - 数据库文件位置：`data/apex.db`
    - 支持历史数据查询与统计分析

### 4.4 Web Dashboard (Web 管理界面)
提供 Web 界面展示 Usage 使用记录与 Metrics 指标：

#### 4.4.1 技术栈
- **前端框架**: Next.js 16 (App Router)
- **UI 组件库**: shadcn/ui
- **样式**: Tailwind CSS
- **构建输出**: 静态文件导出至 `target/web` 目录

#### 4.4.2 数据展示
- **Usage 记录**：展示团队调用明细，包含时间、团队、路由、渠道、模型、输入/输出 Token 数
- **Metrics 指标**：展示请求总数、错误数、FallBack 次数、平均延迟等汇总数据

#### 4.4.2 筛选与查询
- 支持按时间范围筛选
- 支持按 Team / Router / Channel / Model 维度过滤
- 支持分页展示

#### 4.4.3 API 端点
- `GET /api/usage` - 获取 Usage 记录
- `GET /api/metrics` - 获取 Metrics 汇总数据

#### 4.4.5 前端页面
- `GET /dashboard` - Dashboard 主页面
- 使用 Next.js + shadcn/ui 组件库构建
- 支持响应式设计和主题切换

### 4.5 配置热加载 (Hot Reload)
- 监听配置文件变化，自动重载配置。
- 如果新配置非法，保持旧配置生效并记录错误。

### 4.6 MCP Server 操作管理与统计分析
#### 4.6.1 协议与传输
...
### 12.0 Web Dashboard
- **技术栈**：Axum (Rust) + Next.js 16 + shadcn/ui + Tailwind CSS
- **数据层**：SQLite 数据库 (data/apex.db)
- **构建输出**：静态文件导出至 `target/web` 目录
- **安装部署**：通过 `install.sh` 脚本一键部署
- **API 设计**：
  - `GET /api/usage` - 返回 Usage 记录列表，支持 `team_id`, `router`, `channel`, `model`, `limit`, `offset` 查询参数
  - `GET /api/metrics` - 返回 Metrics 汇总数据 (总请求数、总错误数、总 Fallback 次数、平均延迟)
- **前端页面**：
  - `/dashboard` - 主页面，展示 Usage 表格与 Metrics 卡片
  - 支持基础筛选与分页
