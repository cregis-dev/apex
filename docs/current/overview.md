...
**Date:** 2026-03-10
**Type:** Backend (Rust) + Control Plane (Vite + React)
**Architecture:** AI API Gateway with Control Plane and CLI Automation
...
2. **Control Plane (Vite + React)**: 可观测与管理控制台，展示 Usage 记录、Metrics 指标，并支持通道/路由/团队/限流的在线管理
...
- **Location:** `/cp`
- **Purpose:** 可观测与管理控制台，展示使用记录、指标，并支持在线管理
- **Tech Stack:** Vite 6, React 19, React Router 7, TanStack Query

### How Parts Integrate

1. **静态文件服务**: 控制台构建后输出到 `target/web/cp`，由 Rust 后端统一静态资源层提供服务（`/cp`）；发布态推荐通过 `embedded-web` 嵌入到二进制
2. **API 通信**: 控制台通过 `GET /api/dashboard/analytics`、`GET /api/dashboard/records` 等端点与后端通信
3. **认证**: 使用相同的 API Key 体系 (admin / Team-based) 进行认证
...
| UI Library | React 19 |
| Routing | React Router 7 (hash routing) |
...
├── tests/                    # Rust 集成测试
├── cp/                       # 控制台前端 (Vite + React)
│   ├── src/pages/            # 控制台页面 (Overview/Channels/…)
│   ├── src/components/       # 共享组件
│   └── src/lib/              # API 客户端与类型
├── docs/                     # 项目文档
│   ├── current/              # 当前事实文档
