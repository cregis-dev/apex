...
| 字段 | 类型 | 说明 |
|------|------|------|
| `connect_ms` | number | 与上游建立连接的超时（毫秒） |
| `request_ms` | number | **非流式**响应的响应体总时长预算（毫秒） |
| `response_ms` | number | **流式**响应两个数据块之间的最大空闲间隔（毫秒） |

两者不能混用，因为流式和非流式的失败方式不同：

- **流式（SSE）** 响应会持续吐块，所以"长时间没有新块"就意味着连接已死 —— `response_ms`
  用来快速发现这种情况，设小一点（如 30s）是合理的。
- **非流式** 响应相反：上游通常在**开始生成之前**就返回 `200 OK` 响应头，然后等整段
  生成完毕才一次性下发响应体。在模型思考期间响应体本来就是"空闲"的，用空闲超时去卡它
  会误杀正常请求。它需要的是 `request_ms` —— 覆盖整个响应体的总预算，推理模型请留足
  余量（如 300s）。

> 实际生效的非流式预算取 `max(request_ms, response_ms)`。历史上 `response_ms` 也兼管
> 非流式响应体，为兼容那些靠调大 `response_ms` 来适配慢模型的配置，升级后不会因为
> `request_ms` 偏小而突然开始超时。
