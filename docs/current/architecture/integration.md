...
**Scope:** Backend 与控制台 (Control Plane) 的集成架构
...
│  │  /cp, /cp/*   → Static Asset Layer (Control Plane)        │  │
│  └───────────────────────────────────────────────────────────┘  │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │                   Middleware Chain                         │  │
...
**Backend 职责**: 通过统一静态资源访问层提供控制台 (Control Plane) 静态资源
...
7. Browser → GET /cp/
   ↓
8. [Static File Server] → 返回 cp/index.html
   ↓
9. Browser → GET /api/dashboard/analytics
   ↓
10. [Analytics API Handler] → 查询 SQLite
    sql: SELECT * FROM usage_records ...
    ↓
11. Response → Browser (Control Plane UI 展示)
```
...
├─────────────────────────────────────────────────────────────┤
│ $ ./install.sh /opt/apex                                    │
│ → Copy: apex binary (embedded-web) to /opt/apex             │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
...
│ → Serves: API + Control Plane from same port (12356)        │
└─────────────────────────────────────────────────────────────┘
```
...
| `/api/dashboard/analytics` | GET | 控制台分析数据 |
| `/api/dashboard/records` | GET | 控制台明细记录 |
| `/cp`, `/cp/*` | GET | 控制台静态文件 |
...
Apex Gateway
    ...
    └─> SQLite (Usage/Metrics 事件)
        ↓
        Control Plane → /api/dashboard/records → 展示
```
...
Apex Gateway
    ...
    └─> SQLite (聚合数据)
        ↓
        Control Plane → /api/dashboard/analytics → 展示
```
...
A: 先确认发布二进制是否使用 `embedded-web` 构建；文件系统模式下检查 `target/web/cp` 已生成且进程工作目录正确。
